...
export function promptForAnswer(userQuery: string, sqlStatement: string, rowData: RowDataPacket[] | ResultSetHeader): ChatCompletionMessageParam[] {
  return [
    {
      role: "system",
      content: `You are an intelligent AI assitant who specializes in answering questions of our employees based on the data retrieved from our database. Our company name is "EUVIC" and we are a major IT services provider in Poland. You will be provided with:
        
        1. The initial question asked by the employee (in human language - most probably Polish).
        2. Comprehensive schema of our database which contains only a single table - 'zyskow'. The schema will be provided in JSON format.  
